...
class Wrapper:
    ...
    def __run(sensorid:int, startdate:datetime, enddate:datetime):
        ...
        # process data
        sensor_processor = Processor(sensor_data, File_Data_Source.SAP_AND_MOISTURE_SENSOR)
        sensor_processor.remove_fields(['Field','Sensor ID'])
        weather_processor = Processor(weather_data, File_Data_Source.WEATHER_STATION)
    ...
    def __get_almond_sensor_data(sensorid:int, startdate:datetime, enddate:datetime): 
        """parse data in years/months timeframe (needs to read multiple files)
        if an error is thrown here it's probably because a file doesn't exist in ../data"""
        sensor_data = Parser.parse(os.path.join(ROOT_DIR,'data','Data_TREWid{id}_{year}_{month:0>2}_almond.csv'.format(id=sensorid,year=startdate.year%100,month=startdate.month)), File_Data_Source.SAP_AND_MOISTURE_SENSOR)
    ...
    def __get_almond_weather_data(startdate:datetime, enddate:datetime): 
        """parse data in years/months timeframe (needs to read multiple files)
        if an error is thrown here it's probably because a file doesn't exist in ../data"""
        weather_data = Parser.parse(os.path.join(ROOT_DIR,'data','Data_weather_{year}_{month:0>2}_almond.csv'.format(year=startdate.year%100,month=startdate.month)), File_Data_Source.WEATHER_STATION)
        if startdate.year < enddate.year:
            #first year
            if startdate.month == 12:
                pass
