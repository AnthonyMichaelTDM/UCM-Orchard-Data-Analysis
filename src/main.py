...
#choose between sources (almond or pistacio)
config:Configs = None
while isinstance(config, type(None)):
    match get_char("do you want to analyze almond data or pistacio data? (enter 'a' for almond or 'p' for pistacio"):
        case 'a':
            config = Configs.ALMOND
            break
        case 'p':
            config = Configs.PISTACIO
            break
        case _:
            print("invalid input, try again")
            continue
